  ...
  return (
    <div ref={ref} className="relative w-full">
      <button
        ...
        className={`w-full pl-10 pr-4 py-4 text-left bg-white rounded-lg border border-gray-300 focus:outline-none ${
            ...
            {weekDays.map((day) => {
              ...
              return (
                <button
                  ...
                  className={`aspect-square w-full rounded-full flex items-center justify-center transition ${
                    isSelected
        ...
        <PickerPopup onClose={() => setOpen(false)}>
          <div className="w-full">
            ...
            <div className="flex items-center justify-center gap-3">
              <div className="flex flex-col items-center">
                <input
                  ...
                  className="
                    w-16 h-12 text-center text-xl font-semibold text-gray-800
                    bg-gray-50 rounded-lg border border-gray-200
                    focus:outline-none focus:border-[#ff9900]
                    [appearance:textfield]
                    [&::-webkit-inner-spin-button]:appearance-none
                    [&::-webkit-outer-spin-button]:appearance-none
                  "
                />
                <span className="text-[9px] text-gray-400 mt-1.5">HOUR</span>
              </div>
              ...
              <div className="flex flex-col items-center">
                <input
                  ...
                  "
                />
                <span className="text-[9px] text-gray-400 mt-1.5">MINUTE</span>
              </div>
            </div>
            ...
            <button
              ...
              className="
                w-full mt-3 py-2 rounded-lg bg-[#ff9900] text-white text-sm
                font-semibold hover:bg-[#ff8800] transition cursor-pointer
              "
            >
              Done
            </button>
...
function ErrorToast({
  ...
  return (
    <div
      role="alert"
      className="fixed top-6 left-1/2 z-60 flex w-[min(92vw,420px)] -translate-x-1/2 items-start gap-3 rounded-xl border border-red-200 bg-white p-4 shadow-2xl"
    >
      <XCircle className="mt-0.5 h-6 w-6 shrink-0 text-[#ff9900]" />
      <div className="flex-1">
        <p className="font-medium text-[#ff9900]">Reservation failed</p>
...
function Reservation() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!date || !time || !person) {
      setErrorToast("Please select a date, time, and number of guests.");
      return;
    }
    ...
    try {
      const response = await BookTable(name, email, Number(person), time, date);
    ...
    <div className="reservation w-full min-h-[80vh] flex justify-center items-center relative">
      ...
      <div className="relative form bg-white/25 backdrop-blur-[1px] p-6 md:p-12 min-h-130 shadow-[0_20px_60px_rgba(0,0,0,0.35)] flex justify-center items-center">
        ...
        >
          ...
          {/* Inline error message removed – now shown as toast */}
